...
def get_experiment(dataset_name: str, dataset_task: str) -> Tuple[Any, Any, Any]:
    ...
    if True:
        n_sub = 100
        n_tot = dataset_labels["labels"].shape[0]
        n_lab = dataset_labels["labeled_initial_indices"].shape[0]
        subset_indices = torch.concat(
            [
                dataset_labels["labeled_initial_indices"],
                make_subset_indices(0, n_tot, n_sub - n_lab),
            ],
            -1,
        )
        overlap = subset_indices.shape[0] - len(set(x.item() for x in subset_indices))
        if overlap > 0:
            print(f"WARNING: subsampling produced overlap of size {overlap}")

        sub_traces: Dict[str, Any] = {}
        sub_labels = dataset_labels["labels"][subset_indices]
        for k, v in dataset_data["traces"].items():
            sub_traces[k] = v[subset_indices, :, :]

    else:
        sub_traces = dataset_data["traces"]
        sub_labels = dataset_labels["labels"]
    traces: BatchedTrace = (
